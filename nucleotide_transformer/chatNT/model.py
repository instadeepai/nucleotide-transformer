...
class BioBrainDecoder(hk.Module):
    """
    Creates the decoder for the BioBrain model.
...
class BioBrainEncoder(hk.Module):
    """
    Creates the bio encoder for the BioBrain model.
    Mainly the ESM encoder but wrapped inside a module to properly
    control mixed precision.
    """

    def __init__(
        self,
        esm_config: GptConfig,
        esm_name: Optional[str] = None,
        name: Optional[str] = None,
    ):
        """
        Initializes the bio to english projection layer.
        ...
        Args:
            esm_config: config of the ESM model
            esm_name: name for the ESM model
            name: haiku module name
        """
        super().__init__(name=name)
        self._config = esm_config
        self._esm_config = esm_config
    ...
    def __call__(
        ...
        # get ESM model
        esm_model = ESMTransformer(self._esm_config, name=self._esm_name)

        # compute bio seq embeddings

        esm_outs = esm_model(tokens=bio_token_ids)
        num_esm_layers = self._config.num_layers
        bio_embeddings = esm_outs[f"embeddings_{num_esm_layers}"]

        return bio_embeddings


def build_multi_omics_fn(
    ...
    esm_compute_dtype: jnp.dtype = jnp.float32,
    esm_param_dtype: jnp.dtype = jnp.float32,
    esm_output_dtype: jnp.dtype = jnp.float32,
    ...
    """
    Create an English + one bio modality model's forward pass.
    """

    assert {esm_compute_dtype, esm_param_dtype, esm_output_dtype}.issubset(
        {
            jnp.bfloat16,
            jnp.float32,
            jnp.float16,
    ...
    hk.mixed_precision.set_policy(BioBrainDecoder, gpt_policy)

    # set policy for ESM
    esm_policy = jmp.Policy(
        compute_dtype=esm_compute_dtype,
        param_dtype=esm_param_dtype,
        output_dtype=esm_output_dtype,
    )
    hk.mixed_precision.set_policy(BioBrainEncoder, esm_policy)
    ...
    # make sure to get final embeddings with ESM
    num_esm_layers = esm_config.num_layers
    emb_layers_to_save = esm_config.embeddings_layers_to_save + (num_esm_layers,)
    esm_config = replace(esm_config, embeddings_layers_to_save=emb_layers_to_save)
    ...
    def multi_omics_fn(
        ...
        if bio_token_ids is None:
            ...
            # create models
            bio_encoder = BioBrainEncoder(esm_config=esm_config, esm_name=esm_name)
